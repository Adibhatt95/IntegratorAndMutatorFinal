...
namespace IntegratorANDMutator
{
    ...
    {
        public static int tempNewCard = 0;
        ...
        public List<Card> mutateSpecificCard(List<Card> toMutate, Dictionary<int, string> allCards, Dictionary<string, int> cardname, int oldCard)
        {
            Dictionary<int, int> dictToMutate = new Dictionary<int, int>();
            dictToMutate = convertListtoDict(toMutate, cardname);
            bool swapSuccess = false;
            ...
            while (!swapSuccess)
            {
                ...
                if (dictToMutate.ContainsKey(oldCard) && allCards.ContainsKey(newCard) && (oldCard != newCard))
                {
                    Console.WriteLine("in if statement: old card="+oldCard+"and new card="+newCard);
                    if (dictToMutate.ContainsKey(newCard) && dictToMutate[newCard] == 2)
                    {
                        swapSuccess = false;
                    }
                ...
                {
                    swapSuccess = false;
                    Console.WriteLine("in else statement: old card=" + oldCard + "and new card=" + newCard);
                }
                if(tempNewCard == newCard)
                {
                    newCard = rand.Next(0, allCards.Count + 1);
                    tempNewCard = newCard;
                }
            }
            List<Card> mutated = new List<Card>();
